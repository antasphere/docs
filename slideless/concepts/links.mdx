---
title: "Share links"
description: "A share link is a secret URL created on top of a deck. Whoever holds it opens the deck: there is no sign-in on the other side, no visibility setting, and no way to make a link private. Privacy is not minting a link. A deck with no link is reachable only by its owner, the workspace's admins and the collaborators invited on it."
---

{/* GENERATED from antasphere/slideless docs/concepts/links.md — edit there, then run `npm run sync`. */}
...
call reaches the instance from a deck's own script ([viewer security model](/slideless/security/viewer-security-model)).
...
| -------------------------- | ------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Version**                | latest  | `Always the latest version`, or `Pin to a version`; see [Versions](/slideless/concepts/versions)                                                                                                                                                                                                                                                                                                                                                                                 |
| **Allow annotations**      | off     | The recipient can leave notes on the deck, anchored to text, a spot or a region; see [Annotations](/slideless/sharing/annotations)                                                                                                                                                                                                                                                                                                                                   |
| **Allow form submissions** | on      | The recipient can submit the forms the deck embeds; see [Forms](/slideless/sharing/forms)                                                                                                                                                                                                                                                                                                                                                                            |
| **Remember answers**       | on      | The link is its recipient's response: reopening it brings the answers back, every submit updates them, and whoever holds the link can read and change them. Off for a link many people will open. `remembersResponses` on the API and the MCP tool, `--no-remember` / `--remember` on the CLI (an unnamed CLI link is off); embeds never remember; see [Forms](/slideless/sharing/forms)                                                                             |
| **Allow file uploads**     | on      | The recipient can add files to the file fields of the deck's forms. Needs form submissions on. Whoever holds the link can then write files to the instance, inside the instance's size limits. A link created before file fields existed is off until you turn it on. `canUploadFiles` on the API and the MCP tool, `--no-uploads` on the CLI, `slideless uploads <id> <tokenId> --on` for an existing link; see [File fields](/slideless/sharing/forms#file-fields) |
| **Allow downloads**        | on      | The recipient can download the version's files, its `downloads/` folder, one by one or as a zip; see [Attachments](/slideless/concepts/attachments)                                                                                                                                                                                                                                                                                                                              |
...
| **Password**               | none    | At least 4 characters; the recipient types it before the deck opens. Password-protected links do not render inside an [embed](/slideless/sharing/embedding)                                                                                                                                                                                                                                                                                                          |
...
apart from views). [Link analytics](/slideless/sharing/link-analytics) keeps one event per
...
- **The CLI**: `slideless share <id> --name "Alice"` prints the URL; `--to-version`, `--annotator`,
  ...
  mails it. The [CLI reference](/slideless/agents/cli) has every flag.
...
relative to its own URL (`downloads/figures.csv`, `downloads.zip`), as [Attachments](/slideless/concepts/attachments)
...
URL, and the deck's AGENT.md. [Share links, read by agents](/slideless/agents/share-links-for-agents)
...
the direct link except the bar, which never mounts inside a frame; [Embedding decks on your site](/slideless/sharing/embedding) has the snippets and the
limits.
