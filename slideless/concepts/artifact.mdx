---
title: "The deck is the artifact"
description: "A deck is one artifact in your account: one record with a page of its own, a history of immutable versions, and share links created on top of it. Everything you do to a deck happens on that one thing. A link is never the deck, only a door into it."
---

{/* GENERATED from antasphere/slideless docs/concepts/artifact.md — edit there, then run `npm run sync`. */}
...
origin, and it is not what a recipient sees: a recipient sees a [share link](/slideless/concepts/links).
...
[Versions](/slideless/concepts/versions)), **All decks**, and **Delete**.

On the right, for the version the page shows: **Download files** with the file count when that
version carries [attachments](/slideless/concepts/attachments) (**All files (zip)** first, then each file by name),
...
[Deck self-description](/slideless/agents/deck-self-description)). The preview is a picture of the
