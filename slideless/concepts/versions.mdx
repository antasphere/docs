---
title: "Versions"
description: "Every push is a new version of the deck, numbered from 1, and a version never changes once it is committed. The owner keeps the whole history and can show, pull or duplicate any version; a recipient sees one version, the one their link resolves to."
---

{/* GENERATED from antasphere/slideless docs/concepts/versions.md — edit there, then run `npm run sync`. */}
...
carries [attachments](/slideless/concepts/attachments) (a `downloads/` folder).
...
in the share sheet of the [deck's page](/slideless/concepts/artifact), `slideless pin <id> <tokenId> --to-version 2`
...
- **Duplicate** copies one version (the current one from the page, any one through the API) into
  a new deck at version 1; see [The deck is the artifact](/slideless/concepts/artifact).
