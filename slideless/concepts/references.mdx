---
title: "References"
description: "A reference is a deck the workspace keeps to make other decks from. It is a deck like any other: it has versions, files, an owner and share links. What makes it a reference is one line in its AGENT.md. An agent that authors a new deck fetches the reference, reads it, and builds the new deck from what it read."
---

{/* GENERATED from antasphere/slideless docs/concepts/references.md — edit there, then run `npm run sync`. */}
