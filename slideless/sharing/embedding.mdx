---
title: "Embedding decks on your site"
description: "Any share link can render inline on your own website — a landing page, a documentation site, an internal wiki — instead of sending visitors away to the viewer. Paste a small snippet where the deck should appear and it renders there, fully isolated from the rest of your page."
---

{/* GENERATED from antasphere/slideless docs/sharing/embedding.md — edit there, then run `npm run sync`. */}

import { HeroBand } from "/snippets/HeroBand.jsx";

<HeroBand
  eyebrow={"Sharing & review"}
  title={"Embedding decks on your site"}
  drawing={"meridians"}
  seed={2925437878}
  compact={true}
...
Any [share link](/slideless/concepts/links) can render **inline on your own website** — a landing page, a documentation site, an internal wiki — instead of sending visitors away to the viewer. Paste a small snippet where the deck should appear and it renders there, fully isolated from the rest of your page.
...
| -------------------------- | ------------------------------------------------------------------------------------------------------------------ |
| `data-slideless-placement` | Labels this spot in the link's [analytics](/slideless/sharing/link-analytics) — the loader appends it as the `?p=` placement label |
...
- The deck itself is additionally served under `Content-Security-Policy: sandbox` by the viewer, the same double layer the dashboard's own preview uses. The [viewer security model](/slideless/security/viewer-security-model) has the full story.
...
- **Annotations never appear in embeds.** The notes overlay and its badge only mount when the deck is the top-level page, even on a link that allows annotations. Recipients who should annotate need the direct link. [Forms](/slideless/sharing/forms) are the deliberate exception to embedded interactivity (see above).
- **Embeds stay bare.** The recipient bar a share link shows over the deck (its title, its version, the [downloads](/slideless/sharing/downloads)) never mounts inside an embed or any iframe, whatever the link's `showBar` says: the frame belongs to your page.
