---
title: "Annotations"
description: "Annotator share links let reviewers leave notes directly on a shared deck — anchored to the exact text, spot, or region they mark — and give the owner one place to read, resolve, and pull those notes. No reviewer account is needed: the link is the whole credential."
---

{/* GENERATED from antasphere/slideless docs/sharing/annotations.md — edit there, then run `npm run sync`. */}
...
Everything else about share links applies unchanged: pin the link to a version with `--to-version`, protect it with `--password`, expire it with `--expires`. See [Share links](/slideless/concepts/links) for what a link carries and [cli.md](/slideless/agents/cli) for the full sharing reference.
