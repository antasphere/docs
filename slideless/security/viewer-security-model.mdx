---
title: "The viewer security model (for operators)"
description: "Slideless's whole point is rendering untrusted, user-authored HTML to anonymous visitors. This page is what an operator needs to know about how that is made safe, what the guarantees are, and the one hardening knob. (The full platform posture lives in security.md.)"
---

{/* GENERATED from antasphere/slideless docs/security/viewer-security-model.md — edit there, then run `npm run sync`. */}
...
(The full platform posture lives in [security.md](/slideless/security/security).)
...
[`VIEW_DEDUPE_WINDOW_MINUTES`](/slideless/reference/env-reference) (default 10
...
and sets no cookie ([Share links, read by agents](/slideless/agents/share-links-for-agents)).
...
[reverse-proxy.md](/slideless/self-hosting/reverse-proxy). (Per-deck subdomains
...
- Share links with forms on use the same kind of public, token-authed API
  ...
  [security.md](/slideless/security/security#the-form-surface-and-the-files-it-takes) and
  [Forms](/slideless/sharing/forms#file-fields).

## Operator checklist

- Serve the instance over **HTTPS behind a reverse proxy**
  ([reverse-proxy.md](/slideless/self-hosting/reverse-proxy)) — share secrets travel in URLs.
...
- Size the disk for form uploads, or switch them off: each deck can hold up
  ...
  the whole instance
  ([deployment-profiles.md](/slideless/self-hosting/deployment-profiles#sizing-storage-for-form-uploads)).
