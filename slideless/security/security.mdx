...
description: "What a Slideless instance enforces out of the box, what its operator is responsible for, and the rules that govern rendering user content."
---

{/* GENERATED from antasphere/slideless docs/security/security.md — edit there, then run `npm run sync`. */}
...
- **Workspaces are isolated from each other, on one instance too.** Any
  ...
  ([Workspaces](/slideless/concepts/workspaces)); its decks, files, members,
...
- **A project is a grant, and only a grant.** A project
  ([Projects](/slideless/concepts/projects)) opens the decks linked to it to the
...
- **Workspace export + account deletion.** `GET /workspace/export` streams a
  ...
  ([viewer-security-model.md](/slideless/security/viewer-security-model)); for an admin or
...
- **Terminate TLS at a reverse proxy** and set `TRUST_PROXY=true` only there
  ([reverse-proxy.md](/slideless/self-hosting/reverse-proxy)).
...
- `VIEWER_BASE_URL` moves share links onto a dedicated user-content origin —
  ...
  ([viewer-security-model.md](/slideless/security/viewer-security-model)); the sandbox stays
