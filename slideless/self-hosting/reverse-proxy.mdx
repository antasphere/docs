...
description: "TLS terminates at a reverse proxy in front of the app — documented, not bundled. Caddy is the recommended proxy: automatic Let's Encrypt, two lines of config."
---

{/* GENERATED from antasphere/slideless docs/self-hosting/reverse-proxy.md — edit there, then run `npm run sync`. */}
...
([viewer-security-model.md](/slideless/security/viewer-security-model)) is one more site
