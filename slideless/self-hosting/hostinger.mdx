---
title: "Install on Hostinger"
description: "Run Slideless on a dedicated Hostinger VPS. You manage the VPS and its DNS; the template installs Slideless, PostgreSQL, and Caddy for automatic HTTPS. No terminal commands are required for installation."
---

{/* GENERATED from antasphere/slideless docs/self-hosting/hostinger.md — edit there, then run `npm run sync`. */}
...
[Connect an agent](/slideless/getting-started/connect-an-agent) for OAuth or
...
[environment reference](/slideless/reference/env-reference).
