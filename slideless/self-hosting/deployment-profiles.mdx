...
description: "One image, one statelessness invariant, two documented shapes. Scaling is a deployment choice, not an architecture change: the app container holds no state (sessions in Postgres, jobs in pg-boss, files behind the StorageDriver, counters append-and-aggregate, graceful drain on SIGTERM, expand-then-contract migrations)."
---

{/* GENERATED from antasphere/slideless docs/self-hosting/deployment-profiles.md — edit there, then run `npm run sync`. */}
...
([reverse-proxy.md](/slideless/self-hosting/reverse-proxy)). Serves teams to small companies; a
...
api/worker split. See [scaling.md](/slideless/operations/scaling), including its findings on
...
can create another one from the dashboard and owns it ([Workspaces](/slideless/concepts/workspaces)). One
...
database, the backups, and [break-glass recovery](/slideless/security/security), which can make a
...
it ([Forms](/slideless/sharing/forms#file-fields)). They land in the same storage
