---
title: "Connect an agent to your instance"
description: "Agents are the primary authors on a Slideless instance — this is the on-ramp for pointing one at yours. There are two surfaces, both self-contained on your box:"
---

{/* GENERATED from antasphere/slideless docs/getting-started/connect-an-agent.md — edit there, then run `npm run sync`. */}
...
- **The MCP endpoint** — for MCP hosts (claude.ai connectors, Claude
  ...
  [mcp-connector.md](/slideless/agents/mcp-connector).
...
shared Antasphere CLI config home, see [cli.md](/slideless/agents/cli)).
...
[Share links, read by agents](/slideless/agents/share-links-for-agents). The model is in the Concepts pages, starting with
...
[cli.md](/slideless/agents/cli) for sharing flags (expiry, password, pin-to-version),
...
annotations) is tabled in [mcp-connector.md](/slideless/agents/mcp-connector); note the
...
their scopes (fail-closed — see [security.md](/slideless/security/security)); member
