---
title: "The Slideless CLI"
description: "slideless is the typed command-line client for a Slideless instance — the primary human/agent face of it. It signs in over email OTP (minting its own API key), pushes deck folders as immutable versions, pulls them back byte-exactly, manages share links and collaborators, and previews decks locally under the exact viewer sandbox. The model behind the commands (one deck as one artifact with a page of its own, immutable versions, links made on top, files travelling with the version) is in the Concepts pages: The deck is the artifact, Versions, Share links, Attachments. What each release added is the Changelog page of the docs site, generated from the repository's release tags; this page describes the current CLI."
---

{/* GENERATED from antasphere/slideless docs/agents/cli.md — edit there, then run `npm run sync`. */}
...
[Your Antasphere account](/slideless/getting-started/antasphere-account)) and
...
- **The deck's page**: every push answers with the deck's own page on the
  ...
  push; [The deck is the artifact](/slideless/concepts/artifact) describes the
...
- **Attachments**: a `downloads/` folder at the root of the deck is the
  version's attachment set ([Attachments](/slideless/concepts/attachments), and
  the [Downloads](/slideless/sharing/downloads) page for the URLs and the API):
...
and `template` are its two types: [References](/slideless/concepts/references) is the
...
brand. [Projects](/slideless/concepts/projects) is the model — a workspace
...
expiry, password and counts: [Share links](/slideless/concepts/links) is the model,
[Versions](/slideless/concepts/versions) the latest-or-pinned story.
...
([Share links, read by agents](/slideless/agents/share-links-for-agents)).
