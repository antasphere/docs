...
description: "Every instance is MCP-capable at boot: every instance serves a streamable-HTTP MCP endpoint at /mcp, protected by the instance's own built-in OAuth 2.1 authorization server. No companion service, no shared secrets, no aud/resource URL to keep in sync — the resource identifier is derived at boot as PUBLIC_BASE_URL + '/mcp'."
---

{/* GENERATED from antasphere/slideless docs/agents/mcp-connector.md — edit there, then run `npm run sync`. */}
...
| `slideless_list_projects`                    | read  | Cursor-paginated list of the projects you belong to, each with your own `myRole` on it; `archived` lists the archived ones or both. See [Projects](/slideless/concepts/projects)                                                                                                                                                                                                                                  |
...
| `slideless_list_references`                  | read  | The references you can read (your own, the ones published to the workspace, the ones linked to a project you are on), with `reference`, `audience` and `defaultReference` on each; `type` narrows to `brand` or `template`, omitted lists every type; `projectId` keeps one project's; cursor-paginated. See [References](/slideless/concepts/references)                                                         |
...
| `slideless_get_project_brand`                | read  | A project's brand — the brand reference linked to it and flagged by a manager — as `{ brand }`, or `brand: null`; a project you cannot read answers not found. See [Projects](/slideless/concepts/projects)                                                                                                                                                                                                       |
...
| `slideless_add_share_token`                  | write | Mint a share link (name/pin/annotate/forms/uploads/downloads/pdf/bar/expiry/password: `canAnnotate`, `canSubmitForms`, `remembersResponses`, `canUploadFiles`, `canDownload`, `canExportPdf`, `showBar`, see [Share links](/slideless/concepts/links)); returns the viewer URL once                                                                                                                               |
...
[file field](/slideless/sharing/forms#file-fields), and `slideless_list_form_responses`
...
([CLI reference](/slideless/agents/cli)). Like the answers themselves, a file's `field`,
