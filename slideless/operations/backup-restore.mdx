...
description: "What to back up on a self-hosted instance, how to automate dailies, how to restore — and the drill that proves your backups actually work."
---

{/* GENERATED from antasphere/slideless docs/operations/backup-restore.md — edit there, then run `npm run sync`. */}
...
2. **The `/data` volume** — uploaded file blobs (local storage driver), the
   ...
   deck ([deployment-profiles.md](/slideless/self-hosting/deployment-profiles#sizing-storage-for-form-uploads)).
...
the pepper-rotation runbook in [security.md](/slideless/security/security).
