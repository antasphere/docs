---
title: "Install"
description: "Run your own Antasphere hub instance with Docker: a one-liner for a fresh VPS, or a manual clone-and-compose on any machine. Both paths end with the first-boot wizard in the…"
---

{/* GENERATED from antasphere/hub docs/self-hosting/install.md — edit there, then run `npm run sync`. */}

Run your own Antasphere hub instance with Docker: a one-liner for a fresh
VPS, or a manual clone-and-compose on any machine. Both paths end with the
first-boot wizard in the browser.

## One-liner (fresh VPS)

```bash
curl -fsSL https://raw.githubusercontent.com/antasphere/hub/prod/install.sh | \
  sudo bash -s -- --domain account.antasphere.com
```

Installs git + Docker if missing, clones to `/opt/antasphere`, generates
secrets into `.env` (mode 600), starts the stack, and configures UFW
(22/80/443). With a domain, finish by wiring the reverse proxy
([reverse-proxy.md](/antasphere/self-hosting/reverse-proxy)) and setting `TRUST_PROXY=true` in
`.env`.
...
open http://localhost:3000        # setup token: /opt/antasphere/.env
```
...
```bash
git clone https://github.com/antasphere/hub.git hub
cd hub
...
([env-reference.md](/antasphere/reference/env-reference)).

## What's running

| Service | Image                    | Data                                                  |
| ------- | ------------------------ | ----------------------------------------------------- |
| `app`   | `ghcr.io/antasphere/hub` | `app_data` volume → `/data` (files, generated secret) |
