...
description: "One template, one statelessness invariant, two documented shapes. Scaling is a deployment choice, not an architecture change: the app container holds no state (sessions in…"
---

{/* GENERATED from antasphere/hub docs/self-hosting/deployment-profiles.md — edit there, then run `npm run sync`. */}
...
([reverse-proxy.md](/antasphere/self-hosting/reverse-proxy)). Serves teams to small companies; a
...
| pgvector    | included in the compose image                | enable the extension on the managed instance; products run `CREATE EXTENSION` themselves               |
...
api/worker split. See [scaling.md](/antasphere/operations/scaling). Note the
first-boot ordering the drill proved: a fresh database needs one
`all`/`worker` boot before api-only replicas can start.
