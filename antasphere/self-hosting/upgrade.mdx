---
title: "Upgrade"
description: "Upgrading an instance is a pull plus a restart: migrations apply themselves at boot, and data survives in the named volumes."
---

{/* GENERATED from antasphere/hub docs/self-hosting/upgrade.md — edit there, then run `npm run sync`. */}

Upgrading an instance is a pull plus a restart: migrations apply themselves
at boot, and data survives in the named volumes.
...
```bash
APP_IMAGE=ghcr.io/antasphere/hub:1.2.3
```

Tags published per release: `latest`, `X`, `X.Y`, `X.Y.Z`; the `prod` branch
