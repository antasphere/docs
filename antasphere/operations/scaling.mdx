---
title: "Scale drill: proving Profile B"
description: "The deployment profiles claim the image scales horizontally because the app container holds no state. Claims rot; this drill is the executable proof. It stands up real…"
---

{/* GENERATED from antasphere/hub docs/operations/scaling.md — edit there, then run `npm run sync`. */}

The [deployment profiles](/antasphere/self-hosting/deployment-profiles) claim the
image scales horizontally because the app container holds no state. Claims
rot; this drill is the executable proof. It stands up real multi-replica
stacks with Docker Compose and asserts every scaling invariant with direct
evidence (Postgres
...
- Managed-Postgres/PgBouncer behavior (see the pooling caveats in
  [deployment-profiles.md](/antasphere/self-hosting/deployment-profiles)).
- Rolling-deploy drain and expand-then-contract coexistence, which are
  separate proofs outside this drill.
