...
description: "Your Antasphere account is reachable by agents over MCP at https://account.antasphere.com/mcp, protected by the account service's own OAuth sign-in. Nothing to install and no…"
---

{/* GENERATED from antasphere/hub docs/agents/mcp-connector.md — edit there, then run `npm run sync`. */}

Your Antasphere account is reachable by agents over MCP at `https://account.antasphere.com/mcp`,
protected by the account service's own OAuth sign-in. Nothing to install and no secret to copy:
the agent discovers how to sign in on its own, you approve it once, and you can revoke it at any
time.
